    ...
    m_FPGA_SwitchStatus(0)
{
    ...
    ui->tabCamera->installEventFilter(this);
    ui->tabMPU9250->installEventFilter(this);
    ui->tabLightSensor->installEventFilter(this);
...
Dialog::~Dialog()
{
    ...
    // set switch to camera so openCL camera can work
    fpga->SwitchVideoIn(FPGA::VIDEO_IN_CAMERA);
...
void Dialog::TimerHandle(){
    ...
    if (widgetCurrent == ui->tabGsensor){
        ...
        TabIrPolling(fpga);
        ui->tabIR->update();
    }else if (widgetCurrent == ui->tabMPU9250){
        TabMpu9250Polling(fpga);
        ui->tabMPU9250->update();
    }else if (widgetCurrent == ui->tabLightSensor){
        TabAdc9300Polling(fpga);
...
void Dialog::MoveVideo(){
    ...
    if (Widget != ui->tabVideoIn && Widget != ui->tabCamera)
        return;
...
void Dialog::on_tabWidget_currentChanged(int index)
{
    ...
    if (Widget == ui->tabVideoIn || Widget == ui->tabCamera){
        ...
        if (Widget == ui->tabCamera)
            fpga->SwitchVideoIn(FPGA::VIDEO_IN_CAMERA);
         else
            fpga->SwitchVideoIn(FPGA::VIDEO_IN_COMPOSITIVE);
...
void Dialog::on_verticalSliderFocus_sliderMoved(int position)
{
    char szText[32];
    fpga->CameraFocus(position);

    sprintf(szText, "%d", position);
    ui->label_FocusPos->setText(szText);
}
